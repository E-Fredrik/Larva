                ...
                VStack(spacing: 0) {
                    ...
                    ScrollView(.horizontal, showsIndicators: false) {
                        HStack(spacing: 12) {
                            ...
                            CategoryPill(
                                title: "Map Themes",
                                isSelected: selectedCategory == .mapTheme
                            ) {
                                withAnimation(
                                    .spring(response: 0.3, dampingFraction: 0.7)
                                ) {
                            ...
                            CategoryPill(
                                title: "Borders",
                                isSelected: selectedCategory == .avatarBorder
                            ) {
                                withAnimation(
                                    .spring(response: 0.3, dampingFraction: 0.7)
                                ) {
                            ...
                            CategoryPill(
                                title: "App Icons",
                                isSelected: selectedCategory == .appIcon
                            ) {
                                withAnimation(
                                    .spring(response: 0.3, dampingFraction: 0.7)
                                ) {
                        ...
                        LazyVGrid(columns: columns, spacing: 16) {
                            ForEach(filteredItems) { item in
                                ShopItemCard(
                                    ...
                                    canAfford: viewModel.currentUser.points
                                        >= item.cost
                                ) {
                                    withAnimation(
                                        .spring(
    ...
    var body: some View {
        Button(action: action) {
            Text(title)
                ...
                .shadow(
                    color: isSelected ? .clear : Color.black.opacity(0.05),
                    radius: 3,
                    x: 0,
                    y: 2
                )
