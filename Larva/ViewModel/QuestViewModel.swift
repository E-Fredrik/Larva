...
class QuestViewModel: ObservableObject {
    ...
    private func loadDailyQuests() {
        dailyQuests = [
            Quest(id: "q1", title: "Walk 5,000 steps", targetGoal: 5000, currentProgress: 0, rewardPoints: 100),
            Quest(id: "q2", title: "Walk 10,000 steps", targetGoal: 10000, currentProgress: 0, rewardPoints: 250)
        ]
    }
        ...
        for index in dailyQuests.indices {
            ...
            if dailyQuests[index].title.lowercased().contains("steps") {
                ...
                if !dailyQuests[index].isCompleted {
                    dailyQuests[index].currentProgress = min(currentSteps, dailyQuests[index].targetGoal)
                    
                    //If the update flipped it to completed, and we haven't paid out yet
                    if dailyQuests[index].isCompleted && !claimedQuestIDs.contains(questID) {
    ...
    private func autoClaimReward(for questID: String, at index: Int) {
        ...
        print("Claimed: \(dailyQuests[index].title)! Awarded \(dailyQuests[index].rewardPoints) pts.")
